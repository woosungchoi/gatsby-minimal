...
`LAMP`, `LEMP`와 같은 전통적인 `DB`, `PHP`, `CMS`로 수동으로 구축했었는데, 기존 방식과 다르게 완전히 새로운 방식으로 만들어졌기 때문입니다.

속도도 굉장히 빠르고, `그누보드`, `워드프레스`에서 못보던 블로그에 특화된 테마도 많았구요.
...
그래서 홈서버에 `Node.js`를 깔고 직접 고스트 블로그를 운영했었는데요.
...
정적사이트는 전통적인 서버조차도 필요없는 서버리스 기술로써, `Vercel`이나 `Netlify` 같은 대인배 회사에서 무료로 호스팅(배포)됩니다.

게다가 DB가 필요없는 정적파일로만 구성되어 속도도 초광속일 수 밖에 없는 구조입니다.
...
하지만 `Gatsby Casper` 블로그의 경우 최근 업데이트가 없어서 패키지의 보안 문제가 있었습니다.
...
`Minimal` 블로그는 특히 `Gatsby` 개발자가 개발, 유지, 보수하기 때문에 업데이트 걱정이 필요없고,

`RSS`, `Sitemap`, `SEO` 기능이 기본 탑재되어 초보도 쉽게 만들 수 있는 장점이 있습니다.
...
만약에 다른 테마로 넘어간다면 Flexiblog일 것입니다.

Flexiblog에 대한 내용은 <a href="https://blog.wsgvet.com/gatsby-flexyblog" target="_blank" rel="noopener noreferrer">FlexiBlog 괜찮은 것 같습니다</a>를 참조하세요.